...
const AmbulanceETARoute = ({ currentHospitalId, currentHospitalName, hospitalLocation = { lat: 12.8752, lng: 74.8470 } }) => {
        ...
        const initMap = async () => {
            try {
                ...
                mapInstance.current = L.map(mapRef.current, {
                    zoom: 13,
                    center: [KMC_HOSPITAL_LAT, KMC_HOSPITAL_LNG]
                });
                ...
                L.marker([KMC_HOSPITAL_LAT, KMC_HOSPITAL_LNG], { icon: hospitalIcon })
                    .bindPopup(`<b>${KMC_HOSPITAL_NAME}</b><br>Hospital Location`)
                    .addTo(mapInstance.current);
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/ambulance/${selectedAmbulance._id}/start-route`, {
                ...
                body: JSON.stringify({
                    ...
                    destinationLatitude: KMC_HOSPITAL_LAT,
                    destinationLongitude: KMC_HOSPITAL_LNG,
                    destinationAddress: KMC_HOSPITAL_NAME,
    ...
    const handleGetRoute = async () => {
        ...
        try {
            ...
            const osrmUrl = `https://router.project-osrm.org/route/v1/driving/${ambulanceLng},${ambulanceLat};${longitude},${latitude};${KMC_HOSPITAL_LNG},${KMC_HOSPITAL_LAT}?overview=full&steps=true&geometries=geojson`;
            ...
            try {
                ...
                if (data.code === 'Ok' && data.routes && data.routes.length > 0) {
                    ...
                    drawRouteOnMap(routeCoords, ambulanceLat, ambulanceLng);
                    setSuccess(`✓ Route: ${distance.toFixed(1)}km, ~${Math.round(duration)}min via roads`);
                    ...
                    // Auto-predict ETA using ML model
                ...
                clearTimeout(timeoutId);
                console.warn('OSRM failed, using direct line fallback:', osrmErr.message);
                
                // Calculate direct distance as fallback
                const R = 6371; // Earth radius in km
                const dLat = (latitude - ambulanceLat) * Math.PI / 180;
                const dLng = (longitude - ambulanceLng) * Math.PI / 180;
                const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) +
                    Math.cos(ambulanceLat * Math.PI / 180) * Math.cos(latitude * Math.PI / 180) *
                    Math.sin(dLng / 2) * Math.sin(dLng / 2);
                const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
                const directDistance = R * c;
                const estimatedDuration = Math.round((directDistance / 40) * 60); // Assume 40km/h average
                
                const simplePath = [
                    { latitude: ambulanceLat, longitude: ambulanceLng },
                    { latitude: latitude, longitude: longitude },
                    { latitude: KMC_HOSPITAL_LAT, longitude: KMC_HOSPITAL_LNG }
                ];
                drawRouteOnMap(simplePath, ambulanceLat, ambulanceLng);
                setRoutePath(simplePath);
                setSuccess(`⚠ Direct path: ${directDistance.toFixed(1)}km, ~${estimatedDuration}min (routing service unavailable)`);
    ...
    const drawRouteOnMap = (path, ambulanceLat, ambulanceLng) => {
        if (!mapInstance.current) return;

        // Clear existing route line
        if (markersRef.current.routeLine) {
            mapInstance.current.removeLayer(markersRef.current.routeLine);
        }
        ...
        if (path && path.length > 0) {
            ...
            // Draw route line
            markersRef.current.routeLine = L.polyline(pathCoords, {
                color: '#667eea',
                weight: 4,
                opacity: 0.8,
                dashArray: '5, 5'
            ...
            markersRef.current.pickup = L.marker([latitude, longitude], { icon: pickupIcon })
                .bindPopup(`<b>Pickup Location</b><br>${pickupLocation}<br>Lat: ${latitude?.toFixed(4) || 'N/A'}, Lng: ${longitude?.toFixed(4) || 'N/A'}`)
                .addTo(mapInstance.current)
                .openPopup();
            ...
            const allPoints = [
                ...
                ...pathCoords,
                [KMC_HOSPITAL_LAT, KMC_HOSPITAL_LNG]
            ];
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                            ...
                            <div className="ambulance-map-container" style={{ height: '400px' }}>
                                ...
                                <div className="ambulance-map-controls">
                                    ...
                                    <button className="ambulance-map-btn" title="Center" onClick={() => mapInstance.current?.setView([KMC_HOSPITAL_LAT, KMC_HOSPITAL_LNG], 13)}>
